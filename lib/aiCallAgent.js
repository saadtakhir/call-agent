...
Javobda FAQAT "message" bo'lib, unda "juda ko'p... narx oralig'ini... yoki maydon oralig'ini ayta olasizmi?" kabi so'ralsa — natija juda ko'p bo'lgani uchun ("listings"/"categories" YO'Q). Mijozdan taxminiy narx oralig'i (necha so'mdan necha so'mgacha) va/yoki maydon oralig'ini (necha kvadrat metrdan necha kvadrat metrgacha) so'rang — ikkalasi ham shart emas, birortasi kifoya. Mijoz javob berganidan so'ng, get_property_info'ni AYNAN SHU hudud/kategoriya bilan, endi minPrice/maxPrice va/yoki minArea/maxArea parametrlarini ham qo'shib, QAYTA chaqiring — natija kamayguncha shu jarayonni takrorlash mumkin.
