 ...
 * returned by /api/ai-call/start, after which the widget and sip-bridge each
 * end their own call with a single timer (no polling). Also caps how much a
 * call can bill in lib/callUsage.js. */
...
export async function getActiveSession(sessionId) {
  ...
  return session?.active && session.updatedAt.getTime() > Date.now() - STALE_MS ? session : null;
}

export async function isSessionActive(sessionId) {
  return Boolean(await getActiveSession(sessionId));
}
...
/** Marks a call as hangup-requested in the DB (the live push to the call
 * itself is the Supabase Realtime broadcast, see lib/callHangupRealtime.js). */
