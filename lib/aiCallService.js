...
export function toSpokenForm(text) {
  ...
  // A decimal ("5.5 foiz", "2,25 million") reads as "besh butun besh" — left
  // alone, the generic digit rule below would speak it as "besh.besh" with
  // a dropped-in pause. At most 2 fractional digits (and never followed by
  // another digit), so a thousands-dotted "1.305.926" is not mistaken for
  // one; a leading zero in the fraction ("5.05") is spoken as "nol besh".
  result = result.replace(/(\d+)[.,](\d{1,2})(?!\d)/g, (_match, whole, frac) => {
    const fraction = frac.length === 2 && frac[0] === "0" ? `nol ${numberToUzbekWords(Number(frac[1]))}` : numberToUzbekWords(Number(frac));
    return `${numberToUzbekWords(Number(whole))} butun ${fraction}`;
  });
