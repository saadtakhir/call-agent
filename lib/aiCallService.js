...
export function toSpokenForm(text) {
  ...
  // more naturally to ElevenLabs as "zoqlat" — chat display keeps the
  // correctly spelled "zakalat".
  result = result.replace(/\bzakalat\b/gi, "zoqlat");
  // "boshlang'ich narxi o'zida ..." (the single-bidder auction sentence,
  // see buildAuctionReplyTemplate) reads more naturally as "narxning" —
  // scoped to the "narxi o'zida" pair specifically so it doesn't also
  // catch the unrelated "Boshlang'ich narxi <price> so'm" sentence, which
  // is correct as written.
  result = result.replace(/\bnarxi o'zida\b/gi, "narxning o'zida");
  ...
  // combined one — "o'n nol nol", not "o'n nol" or "ming" anything —
  // with the minutes read digit-pair-by-digit when they're "00" (a bare
  // "nol" alone would lose the fact there are two zero digits there).
  result = result.replace(/\b(\d{1,2}):(\d{2})\b/g, (_match, hour, minute) => {
    const hourWords = numberToUzbekWords(Number(hour));
    const minuteWords = minute === "00" ? "nol nol" : numberToUzbekWords(Number(minute));
...
export async function streamElevenLabsSpeech(text) {
  ...
  const url = `https://api.elevenlabs.io/v1/text-to-speech/${config.elevenLabs.voiceId}/stream?output_format=mp3_44100_128`;
...
export async function getCachedGreetingAudio() {
  const currentHash = hashText(CALL_GREETING_TEXT);
