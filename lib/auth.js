...
export function getSessionUser(value) {
  ...
  try {
    ...
    // The env admin is full access by definition (see verifyCredentials) — its
    // cookie is signed, so this can't be forged. Without this, a session issued
    // BEFORE a new permission was added would lack it until the next login.
    if (ENV_ADMIN_USERNAME && data.u === ENV_ADMIN_USERNAME) return { username: data.u, permissions: ALL_PERMISSIONS };
