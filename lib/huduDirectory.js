...
/** Picks out whichever of resolveRegionIds' own output ids are still at the
 * viloyat (province) level — i.e. the caller named a province but never a
 * specific tuman/shahar within it (resolveRegionIds already drops a
 * province id whenever one of its own districts is ALSO present, so
 * anything left here really is province-only). Used by lookupPropertyInfo
 * to ask for a specific district instead of running a province-wide
 * browse search that would almost always match far too many listings. */
export function findProvinceLevelPlaces(ids) {
  return ids.map((id) => PLACE_BY_ID.get(id)).filter((place) => place?.parentId === null);
}
