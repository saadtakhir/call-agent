...
// 3x the bridge's own send interval (120s) — must stay in sync with that;
// too tight here means every ordinary once-in-a-while delay wrongly flags
// as offline.
 ...
 * and picked up by the bridge on its NEXT heartbeat (up to ~120s later) —
 * there's no way to reach into the VPS directly, so the heartbeat it
 * already sends every 120s doubles as the only channel to hand it a
