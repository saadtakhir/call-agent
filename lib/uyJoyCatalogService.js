...
// across uy-joy.uz's own catalog changes. RECREATION_FACILITIES (id 8,
// "Дам олиш базаси") has no confirmed Uzbek phrasing yet — falls through
// to a transliteration of the raw Cyrillic name instead of guessing one.
...
function formatBuildingType(buildingType) {
  ...
  return BUILDING_TYPE_LABELS[buildingType.code] || toLatin(buildingType.name) || null;
}
...
export async function fetchPropertyDetail(id) {
  try {
    ...
    if (!res.ok) return { description: NO_DESCRIPTION_TEXT, mahallaName: null, auctionInfo: null };
    ...
    return { description: NO_DESCRIPTION_TEXT, mahallaName: null, auctionInfo: null };
  }
}
