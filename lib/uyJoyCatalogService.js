 ...
 * (national id), bank account numbers, and legal documents) purely to pull
 * out the one field actually needed: a sale-terms description — everything
 * else in that response is discarded immediately and never returned,
 * logged, or passed anywhere.
 *
 * This lives in TWO different places depending on the listing, confirmed
 * live against real data: `productOrder.description` carries the
...
export async function lookupPropertyInfo({ search = "", regionsRaw = "", categoryRaw = null } = {}) {
  ...
  try {
    ...
    sanitized.description = await fetchSaleDescription(first.id);
    sanitized.spokenReply = buildPropertyReplyTemplate(sanitized, first.category?.id, isSpecificLookup);
