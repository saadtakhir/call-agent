...
function buildAuctionReplyTemplate(order) {
  ...
  const auctionDate = order?.product?.advertisingDate ? computeAuctionDate(order.product.advertisingDate, order.daysToStart) : null;
...
export async function fetchPropertyDetail(id) {
  try {
    ...
    return {
      ...
      auctionInfo: buildAuctionReplyTemplate(raw?.productOrder),
    };
  } catch {
