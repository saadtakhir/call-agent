...
// A voice conversation can't usefully read out more listings than this —
// matches summarizeListings' own cap. Once a browse-style search comes
// back with more than this many matches and no price/area filter has been
// applied yet, lookupPropertyInfo asks the caller to narrow it down by
// budget/size instead of silently only reading the first 8 of, say, 40.
    ...
    if (!isSpecificLookup && total > 1) {
      ...
      if (total > LISTING_LIMIT && !hasNarrowingFilter) {
        ...
        return {
          ...
          body: {
            total,
            message: `Jami ${total} ta mos mulk topildi — bu ko'p, aniqroq izlash uchun taxminiy narx oralig'ini (necha so'mdan necha so'mgacha) yoki maydon oralig'ini (necha kvadrat metrdan necha kvadrat metrgacha) ayta olasizmi?`,
          },
        };
      }
