 ...
 * productOrder, not at the top level the other two categories rely on.
 * Checks the top-level field first regardless, in case some tijorat bino
 * listings do have it there like the other categories. */
export function buildTijoratBinoReplyTemplate(raw, mahallaName, isSpecificLookup) {
  ...
  const regionName = toLatin(region?.name) || null;
  const province = toLatin(region?.parent?.name) || null;
  ...
  const price = formatPriceGrouped(raw.price ?? raw.productOrder?.price);
...
export async function lookupPropertyInfo({ search = "", regionsRaw = "", categoryRaw = null } = {}) {
  ...
  try {
    ...
    if (categoryId === CATEGORY_IDS.kvartira) {
      sanitized.spokenReply = buildKvartiraReplyTemplate(first, detail.mahallaName, isSpecificLookup);
    } else if (categoryId === CATEGORY_IDS.xususiyUy) {
      sanitized.spokenReply = buildXususiyUyReplyTemplate(first, detail.mahallaName, isSpecificLookup);
    } else if (categoryId === CATEGORY_IDS.tijoratBino && detail.raw) {
      sanitized.spokenReply = buildTijoratBinoReplyTemplate(detail.raw, detail.mahallaName, isSpecificLookup);
    } else {
