...
/** Fire-and-forget-but-not-actually-dropped: schedules the write to run
 * after the HTTP response has already been sent, via Next's after() —
 * unlike a bare unawaited promise, this is guaranteed to still run to
 * completion even on a serverless function that would otherwise be frozen
 * the instant the response goes out. This is analytics/diagnostics only,
 * so a caller never needs to wait on it — every call site here used to
 * `await` it, adding a DB round trip to the critical path of every single
 * tool-call turn for no benefit the caller could ever see. */
function logLookup(data) {
  after(() => prisma.propertyLookupLog.create({ data }).catch(() => {}));
}
...
// Every uy-joy.uz request below gives up after this long instead of hanging.
export const UY_JOY_TIMEOUT_MS = 8000;
...
export async function searchCatalog({
  ...
  const res = await fetch(config.uyJoyCatalogUrl, {
    ...
    cache: "no-store",
    // Without a timeout, uy-joy.uz being down or hanging leaves the whole
    // call turn waiting until Vercel kills the function — the caller just
    // hears nothing. Failing fast lets the agent say something instead.
    signal: AbortSignal.timeout(UY_JOY_TIMEOUT_MS),
  });
...
export function formatPriceForSpeech(rawPrice) {
...
export function formatAreaForSpeech(rawValue) {
...
/** Bare mahalla name (no "MFY" suffix) for one listing id — a lighter,
 * single-field fetch of the per-id detail endpoint than fetchPropertyDetail
 * (which also builds sale-terms/auction text this list-preview use has no
 * need for). The catalog search item itself never carries `mfy` at all
 * (confirmed live), so there's no way to get this without the extra
 * per-item request. Stripped of "MFY" here (unlike formatMahallaName, kept
 * for the single-property templates) since the caller phrases its own
 * "... mahallasida" around it — leaving "MFY" in would double up once
 * toSpokenForm's own "MFY" -> "mahallasi" rule also runs on it. */
async function fetchBareMahallaName(id) {
  try {
    const res = await fetch(`${config.uyJoyApiBase}/${encodeURIComponent(id)}`, { headers: { Accept: "application/json" }, cache: "no-store", signal: AbortSignal.timeout(UY_JOY_TIMEOUT_MS) });
    if (!res.ok) return null;
    const raw = await res.json();
    return formatMahallaName(raw?.mfy?.name)?.replace(/\s*mfy$/i, "").trim() || null;
  } catch {
    return null;
  }
}

/** A compact, safe-to-speak list (id/price/mahalla only, no PII-adjacent
 ...
 * conversation can't usefully read out more than that anyway. Mahalla
 * (not the catalog item's own `region`, which is often street-level or
 * otherwise inconsistent — same reasoning as extractRegionInfo) is
 * fetched per item in parallel, since the caller needs a real one to say
 * "narx" isn't the only distinguishing detail in a list of several.
 *
 * Price uses formatPriceGrouped (digit form, e.g. "650 000 000"), NOT
 * formatPriceForSpeech (words) — same digit-first approach
 * buildKvartiraReplyTemplate/buildXususiyUyReplyTemplate already use below,
 * and for the same reason: voice callers still hear it spoken correctly,
 * since app/api/ai-call/turn/route.js's toSpokenForm(reply) converts any
 * space-grouped digit run to words right before TTS — but the Telegram
 * channel (app/api/telegram/webhook/route.js), which never calls
 * toSpokenForm at all, now shows the actual digits instead of a wall of
 * spelled-out words, which is what a text chat should show. */
export async function summarizeListings(items, maxShown = 8) {
  const limited = items.slice(0, maxShown);
  return Promise.all(
    limited.map(async (item) => {
      const grouped = formatPriceGrouped(item.price);
      return {
        id: item.id,
        price: grouped ? `${grouped} so'm` : null,
        mahalla: await fetchBareMahallaName(item.id),
      };
    })
  );
}
...
function toLatin(text) {
  // The caption variant ("g'" instead of "ğ" for ғ) — this feeds both the
  // on-screen chat text and, via toSpokenForm later, the TTS text, and
  // "ğ" displayed on screen read oddly (e.g. "Bulunğur" instead of
  // "Bulung'ur") for something meant to be shown, not just narrated.
  return text ? cyrillicToLatinUzCaption(text) : text;
}
...
export function formatPriceGrouped(rawPrice) {
  const n = Number(rawPrice);
  if (!Number.isFinite(n)) return null;
  return Math.ceil(n)
    .toString()
...
const FOUND_PROPERTY_CLOSING_QUESTION = "Sotib olishga qiziqasizmi, sotuv shartlari kerakmi?";

/** Viloyat/tuman — ALWAYS from productOrder.region, per the user, never
 * the response's own top-level `region`. Confirmed live why: top-level
 * `region` sometimes sits one level deeper than expected (a real tijorat
 * bino listing had it pointing at the MAHALLA itself — regionType
 * "MICRO_REGION" — with the tuman as ITS parent and viloyat as the
 * grandparent), which silently lost the viloyat and duplicated the
 * mahalla name (also supplied separately via raw.mfy, see
 * formatMahallaName). productOrder.region doesn't have that problem — a
 * plain, consistent 2-level {name, parent:{name}} on every listing
 * checked so far, regardless of category. */
function extractRegionInfo(raw) {
  const region = raw.productOrder?.region;
  return {
...
export function buildKvartiraReplyTemplate(raw, mahallaName, isSpecificLookup) {
  ...
  if (floorsBuilding != null || floors != null) {
    ...
    // uy-joy.uz uses floors = -1 as its own convention for "basement",
    // not an actual negative floor number — read out accordingly instead
    // of the confusing "joylashgan qavati -1".
    if (floors != null) floorParts.push(Number(floors) === -1 ? "yerto'lada joylashgan" : `joylashgan qavati ${floors}`);
 ...
 * unchanged. */
export function reformatSaleTermsDescription(rawText) {
  let text = rawText.trim();
  text = text.replace(/;\s*/g, ", ");
  // Some listings type the month unit with the wrong letters ("120 оу" in
  // Cyrillic, or Latin "ou", for "oy" — real example: listing 142130), which
  // would otherwise be read out as "ou". Only touched right after a number.
  text = text.replace(/(\d+)\s*(?:[оo][уuйy])(?=[\s).,;]|$)/gi, "$1 oy");
  text = text.replace(
    new RegExp(`(\\d+)\\s*oy\\s*\\(\\s*(\\d+)\\s*oy\\s*imtiyoz\\s*\\)\\s*muddatga`, "gi"),
    "$1 oyga muddatga, shundan $2 oy imtiyozli davr bilan"
  );
  ...
  // Some listings just stop at "...sharti bilan" — finish the sentence the
  // same way the standard closing clause above reads.
  text = text.replace(/sharti bilan\s*\.*\s*$/i, "sharti bilan savdoga chiqarilgan");
  text = text.replace(/\.*\s*$/, ".");
...
function buildAuctionReplyTemplate(raw, order) {
  ...
  if (bidStep != null) {
    sentences.push(
      `Ikki va undan ortiq ishtirokchi bo'lib qolsa, savdo qadami ${bidStep}% bo'ladi va savdo kim oshdi savdo ko'rinishida bo'lib o'tadi, agar faqat bitta odam savdoda qatnashsa, boshlang'ich narxning o'zida to'g'ridan to'g'ri g'olib deb topiladi.`
    );
  }
...
export async function lookupPropertyInfo({
  ...
  minArea = null,
  maxArea = null,
  // Admin-tunable from the Toollar settings tab (see lib/aiTools.js) —
  // defaults below are used when nothing was saved there.
  limits = {},
} = {}) {
  const listingLimit = limits.listingLimit ?? LISTING_LIMIT;
  const maxListingsShown = limits.maxListingsShown ?? 8;
  ...
  if (!listingId) {
    ...
    if (provinceLevel.length) {
      ...
      logLookup({ search, resolvedSearch, ...logExtra, found: true, title: `Tuman aniqlashtirish so'raldi: ${provinceNames}` });
      return {
        status: 200,
        body: {
  ...
  try {
    ...
    if (!items.length) {
      ...
      if (listingId) {
        ...
        if (sold.items.length) {
          logLookup({ search, resolvedSearch, ...logExtra, found: false, title: "Sotilgan" });
          return { status: 404, body: { error: "Bu mulk allaqachon sotilgan." } };
        }
      }
      logLookup({ search, resolvedSearch, ...logExtra, found: false, title: null });
    ...
    if (!isSpecificLookup && total > 1) {
      if (category === null) {
        ...
        logLookup({ search, resolvedSearch, ...logExtra, found: true, title: `Taqsimot: ${total} ta (kategoriya so'ralmagan)` });
        return {
          status: 200,
          body: {
            total,
      ...
      // Checked against items.length too, not just the API's own `total`
      // field — uy-joy.uz's reported total has been observed to undercount
      // relative to the actual items array it returns in the same response
      // (e.g. a real 8-item result reporting total<=5), which would
      // otherwise dump a long, un-narrowed list instead of asking for a
      // budget; items.length is exactly what's about to be shown, so it
      // can't itself be wrong the way a separate reported count can.
      const effectiveTotal = Math.max(total, items.length);
      if (effectiveTotal > listingLimit && !hasNarrowingFilter) {
        logLookup({ search, resolvedSearch, ...logExtra, found: true, title: `Ko'p (${effectiveTotal} ta) — narx/maydon so'raldi` });
        return {
          status: 200,
          body: {
            total: effectiveTotal,
            message: `Jami ${effectiveTotal} ta mos mulk topildi — bu ko'p, taxminan necha so'mgacha byudjetingiz bor?`,
          },
        };
      }

      const listings = await summarizeListings(items, maxListingsShown);
      logLookup({ search, resolvedSearch, ...logExtra, found: true, title: `Ro'yxat: ${total} ta (kategoriya berilgan)` });
      return {
        status: 200,
        body: {
          total,
          listings,
          message: `Jami ${total} ta mos mulk topildi. Har birini "{mahalla} mahallasida - {narx} so'm" shaklida ayting, mijoz tanlagandan keyin o'sha ID bilan qayta so'rang.`,
        },
      };
    }

    const first = items[0];
    logLookup({ search, resolvedSearch, ...logExtra, found: true, title: first.name || null });
